...
class RobotEngineTest : DatabaseBackedTest() {
    ...
    fun `all registers are cleared with 4 damage`() {
        ...
        assertNull(database.cardsForRobot(robot.id, 1))
        assertNull(database.cardsForRobot(robot.id, 2))
        assertNull(database.cardsForRobot(robot.id, 3))
        assertNull(database.cardsForRobot(robot.id, 4))
        assertNull(database.cardsForRobot(robot.id, 5))
    }
    ...
    fun `register 5 is locked with 5 damage`() {
        ...
        assertNull(database.cardsForRobot(robot.id, 3))
        assertNull(database.cardsForRobot(robot.id, 4))
        assertEquals(5, database.cardsForRobot(robot.id, 5)?.id)
    }
    ...
    fun `registers over 4 are locked with 6 damage`() {
        ...
        assertNull(database.cardsForRobot(robot.id, 2))
        assertNull(database.cardsForRobot(robot.id, 3))
        assertEquals(4, database.cardsForRobot(robot.id, 4)?.id)
        assertEquals(5, database.cardsForRobot(robot.id, 5)?.id)
    }
    ...
    fun `registers over 3 are locked with 7 damage`() {
        ...
        assertNull(database.cardsForRobot(robot.id, 1))
        assertNull(database.cardsForRobot(robot.id, 2))
        assertEquals(3, database.cardsForRobot(robot.id, 3)?.id)
        assertEquals(4, database.cardsForRobot(robot.id, 4)?.id)
        assertEquals(5, database.cardsForRobot(robot.id, 5)?.id)
    }
    ...
    fun `registers over 2 are locked with 8 damage`() {
        ...
        robotEngine.clearRegisters(robot)
        assertNull(database.cardsForRobot(robot.id, 1))
        assertEquals(2, database.cardsForRobot(robot.id, 2)?.id)
        assertEquals(3, database.cardsForRobot(robot.id, 3)?.id)
        assertEquals(4, database.cardsForRobot(robot.id, 4)?.id)
        assertEquals(5, database.cardsForRobot(robot.id, 5)?.id)
    }
    ...
    fun `all registers are locked with 9 damage`() {
        robot.damage = 9
        robotEngine.clearRegisters(robot)
        assertEquals(1, database.cardsForRobot(robot.id, 1)?.id)
        assertEquals(2, database.cardsForRobot(robot.id, 2)?.id)
        assertEquals(3, database.cardsForRobot(robot.id, 3)?.id)
        assertEquals(4, database.cardsForRobot(robot.id, 4)?.id)
        assertEquals(5, database.cardsForRobot(robot.id, 5)?.id)
